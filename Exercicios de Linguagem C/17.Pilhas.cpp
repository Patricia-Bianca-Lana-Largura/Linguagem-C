...
//Ddeclaração do tipo_pilha: estrutura de dados.
...
void empilha(int elemento){
	//Verificando se existe alguma posição disponível para inserção.
	if(pilha.topo == tamanho){
		//Se pilha estiver cheia, não é possível empilhar.
		printf("Pilha cheia!\n");
		system("pause");
		return;
	}
	else{
		//Se pilha não estiver cheia.
...
int desempilha(){
	...
	//Verificando se existe algum elemento disponível para remoção
	if(pilha.topo == pilha.ini){
		//Se pilha estiver vazia, não será possível desempilhar
		printf("Pilha vazia.\n");
		system("pause");
		return -1;
